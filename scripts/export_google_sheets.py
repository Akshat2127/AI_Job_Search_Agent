...
try:
    jobs = list_jobs(db)
    rows = [{"id":j.id,"company":j.company,"title":j.title,"location":j.location,"fit_score":j.fit_score,"decision":j.decision,"status":j.status,"url":j.url,"score_reason":j.score_reason,"resume_variant":j.resume_variant} for j in jobs]
